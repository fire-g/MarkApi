...
func (r *Router) Static(pattern string, dir string) {
	...
	pattern = pattern + "(.+)"
	r.AddRoute(GET, pattern, func(w http.ResponseWriter, r *http.Request) {
...
func (r *Router) FilterParam(param string, filter http.HandlerFunc) {
	...
	r.Filter(func(w http.ResponseWriter, r *http.Request) {
...
func (r *Router) ServeHTTP(rw http.ResponseWriter, request *http.Request) {
	...
	for _, route := range r.routes {
		...
		if len(route.params) > 0 {
			...
			request.URL.RawQuery = url.Values(values).Encode() + "&" + request.URL.RawQuery
